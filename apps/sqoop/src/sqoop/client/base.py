...
class SqoopClient(object):
  ...
  def __init__(self, url, user=None, language='en'):
    self._url = url
  ...
  def headers(self):
    ...
    if self._user:
      headers['sqoop-user-name'] = self._user.username
    return headers

  def get_version(self):
    resp = self._root.get('version', headers=self.headers)
    return resp
